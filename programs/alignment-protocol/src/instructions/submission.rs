...
pub fn submit_data_to_topic(
    ...
    link.commit_phase_end = current_time.checked_add(ctx.accounts.topic.commit_phase_duration).ok_or(ErrorCode::Overflow)?;
    link.reveal_phase_start = link.commit_phase_end;
    link.reveal_phase_end = link.reveal_phase_start.checked_add(ctx.accounts.topic.reveal_phase_duration).ok_or(ErrorCode::Overflow)?;
