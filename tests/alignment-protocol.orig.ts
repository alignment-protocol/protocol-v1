...
describe("alignment-protocol", () => {
  ...
  const secretKeyString = fs.readFileSync(
    "/Users/cheul/.config/solana/id.json",
    "utf8"
  );
  ...
  it("Creates user profiles for contributor and validator", async () => {
    ...
    const contributorProfile = await program.account.userProfile.fetch(
      contributorProfilePda
    );
    expect(contributorProfile.user.toString()).to.equal(
      contributorKeypair.publicKey.toString()
    );
    ...
    const validatorProfile = await program.account.userProfile.fetch(
      validatorProfilePda
    );
    expect(validatorProfile.user.toString()).to.equal(
      validatorKeypair.publicKey.toString()
    );
  ...
  it("Submits data to the first topic", async () => {
    ...
    const linkAcc = await program.account.submissionTopicLink.fetch(
      submissionTopicLinkPda
    );
  ...
  it("Stakes tempAlign tokens for tempRep tokens", async () => {
    ...
    const stateAfterValidatorSubmission = await program.account.state.fetch(
      statePda
    );
    console.log(
    ...
    expect(validatorTopicTokenEntry.token.tempAlignAmount.toNumber()).to.equal(
      100 - validatorStakeAmount
    );
  ...
  it("Commits a vote on the submission", async () => {
    ...
    expect(voteCommitAcc.submissionTopicLink.toString()).to.equal(
      submissionTopicLinkPda.toString()
    );
  ...
  it("Finalizes the vote", async () => {
    ...
    const repData = await getAccount(provider.connection, validatorRepAta);
